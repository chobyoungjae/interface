...
export default function HomePage() {
  ...
  const [productExpiry, setProductExpiry] = useState<string>(
    getDefaultExpiry()
  );
  ...
  const [materialInputs, setMaterialInputs] = useState<
    Record<
      string,
      { serialLot: string; stockQuantity: string; quantity: number }
    >
  >({});
  const [serialLotData, setSerialLotData] = useState<
  ...
  const handleMaterialInput = (
    materialCode: string,
    field: "serialLot" | "stockQuantity" | "quantity",
    value: string | number
  ) => {
  ...
  const handleMaterialCopy = (materialCode: string) => {
    const originalMaterial = calculatedMaterials.find(
    ...
    const newKey = `${materialCode}_copy_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 9)}`;
  ...
  const handleMaterialDelete = (materialCode: string) => {
    ...
    setCalculatedMaterials((prev) =>
      prev.filter((m) => m.code !== materialCode)
    );
    ...
    setMaterialInputs((prev) => {
      ...
      if (oldInputs) {
        ...
        newInputs[newCode] = {
          ...
          stockQuantity: "",
          quantity:
            calculatedMaterials.find((m) => m.code === oldCode)?.quantity || 0,
        };
      }
  ...
  const isFormValid = () => {
    ...
    return calculatedMaterials.every(
      (material) =>
        materialInputs[material.code]?.serialLot &&
        materialInputs[material.code]?.stockQuantity
    );
  };
    ...
    try {
      const productionData: ProductionData = {
        ...
        materials: calculatedMaterials.map((material) => ({
          code: material.code.includes("_copy_")
            ? material.code.split("_copy_")[0]
            : material.code,
          name: material.name || "",
          calculatedWeight:
            materialInputs[material.code]?.quantity ?? material.quantity,
            ...
            <div>
              ...
              {selectedProduct && (
                <p className="text-xs text-gray-500 mt-1">
                  기준 생산량:{" "}
                  {(selectedProduct.baseQuantity * 1000).toLocaleString()}g
                </p>
              )}
            </div>
            ...
            <div>
                ...
                <div className="text-lg font-bold text-blue-600">
                  ...
                  {Math.round(
                    calculatedMaterials.reduce(
                      (total, material) =>
                        total +
                        (materialInputs[material.code]?.quantity ??
                          material.quantity),
                      0
                    ) * 1000
                  ).toLocaleString()}
              ...
              <div className="hidden lg:block p-3 font-semibold text-gray-700">
                <div className="flex items-center gap-4">
                  <div className="flex-1 min-w-0">원재료명</div>
                  <div className="flex items-center gap-1 w-24">중량(g)</div>
                  <div className="w-40">시리얼로트</div>
                  <div className="w-32">재고수량(g)</div>
                  <div className="flex items-center gap-1">복사/삭제</div>
                </div>
              </div>
              ...
              <div className="space-y-2 mb-6">
                {calculatedMaterials.map((material, index) => (
                  <MaterialCard
                    ...
                    onSerialLotChange={(serialLot) =>
                      handleMaterialInput(material.code, "serialLot", serialLot)
                    }
                    onStockQuantityChange={(stockQuantity) =>
                      handleMaterialInput(
                        material.code,
                        "stockQuantity",
                        stockQuantity
                    ...
                    onQuantityChange={(quantity) =>
                      handleMaterialInput(material.code, "quantity", quantity)
                    }
                    onMaterialChange={(newCode, newName) =>
                      handleMaterialChange(material.code, newCode, newName)
                    }
